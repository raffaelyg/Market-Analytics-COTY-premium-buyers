# Strategic Market Diagnostic: Ultra-Premium & Luxury Skincare Opportunities

## 📌 Executive Summary
This project was commissioned by Coty to evaluate the post-pandemic recovery and long-term potential of the Ultra-Premium skincare market in Urban China. While the broader beauty market faced significant headwinds in early 2023, this analysis identified a critical Market Polarisation: a surge in consumption at the extreme ends of the price spectrum (Ultra-Luxury and Mass). By integrating Medical Cosmetology (MC) adoption data with longitudinal purchasing behaviour, we mapped the "New Luxury" landscape and identified functional white spaces for portfolio growth.

## 💡 The "One Question"
"How does the rapid adoption of Medical Cosmetology change the 'Job to be Done' for luxury skincare, and where is the friction in current ultra-premium routines?"

## 🛠️ Project Meta: Technical Specifications
| Category	| Key Facts |
| --- | --- |
| Project	| Coty China Ultra-Premium Skincare Market Overview |
| Timeline | 	Completed June 2023 | 
| Primary Data	| Kantar Worldpanel Beauty (Continuous Tracking) | 
| Sample Size	| 45,000 Consumers (2:8 Male/Female mix, Age 15–64) | 
| Sample Scope	| Urban Mainland China (20 Provinces, 4 Municipalities) | 
| Methodology	 | Growth Attribution, Switching Interaction, MC Synergy Analysis | 
| Core Framework	| Market Polarisation & Functional "Job" Mapping | 
| Key Finding	| 1 in 8 Tier 1-4 females use Medical Cosmetology; High synergy with Repair/Lifting

## 🧠 Strategic Insights: The New Luxury Landscape

The analysis moved beyond simple market share to decode the underlying shifts in consumer behaviour and expectations:

* **Market Polarisation**: Consumer inflow is concentrating into Ultra-Premium and Mass segments, leaving "Mainstream Premium" brands in a competitive squeeze. Value growth in Ultra-Premium is driven by both increased penetration and higher frequency.

* **The Medical Cosmetology (MC) Synergy**: 8.8% of target females (1 in 8) have undergone professional MC services. These "High-Value" consumers use skincare not just for maintenance, but for post-treatment recovery and efficacy prolongation, particularly in the "Repair" and "Lifting" sub-segments.

* **Functional White Spaces**: While "Hydration" is a universal entry point, Ultra-Premium growth is centred on Rejuvenation and Anti-ageing. The "Essence Water" and "Cream" categories act as the primary recruitment vehicles for high-spending cohorts.

* **Growth Attribution**: Analysis of top-tier brands (e.g., La Mer, Helena Rubinstein) shows that growth is primarily attributed to Volume per Trip and Frequency, indicating a deeply ingrained habit loop among core loyalists.

## 🖼️ Strategic Gallery: Top 10 Insights
Note: Sensitive market data has been indexed or jittered to protect professional confidentiality.

| No. | Focus Area	| Visual Component	| Key Executive Insight | 
| :---: | --- | --- | --- |
| 01	| Polarisation	| ![Market Context](/images/Slide1.jpeg)	|  **Segment Inflow**: Data proving the shift toward the extreme ends of the price ladder. | 
| 02	| Traction Centre	| ![Polarisation](/images/Slide2.jpeg)	| **Cosmetological Medicine**: A new emerging beauty service that is going to lead how beauty consumers use skincare. |
| 03	| Category Mix	| ![MC Penetration](/images/Slide3.jpeg)	| **Traction Centres**: Essence Water and Creams as the dominant Ultra-Premium entry points. |
| 04	| Category by Pen%	| ![Category Mix](/images/Slide4.jpeg)	| **Traction Centres**: Understanding segments true popularity in the past year by number of buyers. |
| 05	| Growth Attribution	| ![Growth Attribution](/images/Slide5.jpeg)	| **Opportunity Defining**: Finding out which segment holds the most growth potential for buyer extension and upselling space. |
| 06	| Switching Logic	| ![Switching Logic](/images/Slide6.jpeg)	| **Competitive Cohorts**: by Age Consumption and Penetration analysis, Finding the next target audience. |
| 07	| Seasonality Highlights	| ![Functional Matrix](/images/Slide7.jpeg)	| **Demand Mapping**: Identifying the specific seasonal rush for smapling and gifting demands. |
| 08	| Promotion Effectiveness	| ![Price Dynamics](/images/Slide8.jpeg)	| **Promotion Effectiveness**: Data ahve shown that buyers entered via samples and gifts are likely to become new buyers. |
| 09	| Benchmarking	| ![Benchmarking](/images/Slide9.jpeg)  | **Share of Wallet**: Benchmarking Coty’s potential vs. established leaders (Re-Nutriv, Absolue). |
| 10	| Travel Retail	| ![Benchmarking](/images/Slide10.jpeg)  | **Travel Retail**: For high-end brands, travel retail is an essential touch-point where competitors have unanimous strength there. |

## 🚀 Final Results & Recommendations
* **Strategic Pivot**: Recommended that Coty focus functional communication on "Medical-Grade Repair" to capture the high-growth MC-user cohort.

* **Portfolio Optimisation**: Identified Tier 1 and Regional Centre markets as the critical battlegrounds for Ultra-Premium recruitment, where consumer confidence is most resilient.

* **Category Entry**: Advised a "Serum + Cream" duo strategy to address the specific "Moment of Struggle" related to skin barrier recovery post-medical treatment.

## 📄 NDA & Data Privacy

This project was completed for Coty as part of a strategic market overview. All proprietary brand data and financial figures have been anonymised or indexed. The insights shared focus exclusively on the analytical methodology, market modelling, and strategic frameworks.

Contact: [ Rafffael Yuliang Gao / [LinkedIn](https://www.linkedin.com/in/raffaelyg/)]

Location: Toronto, ON, Canada.
